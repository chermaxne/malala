...
export class RecoveryManager {
     ...
     */
    async enableRecovery(wallet, signers, quorum) {
        const signerEntries = signers.map(address => ({
            SignerEntry: {
                Account: address,
                SignerWeight: 1
            }
     ...
     */
    async signForRecovery(txJSON, signerWallet) {
        try {
     ...
     */
    async initiateRecovery(recoveredAccount, newRegularKey) {
        try {
            const client = this.client.getClient();
     ...
     */
    async combineSignatures(signedBlobs) {
        try {
            const client = this.client.getClient();
        ...
        try {
            ...
            try {
                ...
                if (signerResponse.result.account_objects.length > 0) {
                    const signerListObj = signerResponse.result.account_objects[0];
                    signerList = signerListObj.SignerEntries?.map((entry) => ({
